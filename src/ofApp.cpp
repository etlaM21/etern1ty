...
void ofApp::setup(){
	...
	for (int i = 0; i < arguments.size(); i++) {
		if (arguments[i] == "--painter" && i + 1 < arguments.size()) {
			painterName = arguments[i + 1];
		}
	}

	ofBackground(255);

	// 2. Setup the chosen palette and painter
	if (painterName == "heinrich") {
		...
		ofLogNotice() << "Loaded Painter: Heinrich";
	} else { // Christel
		...
		ofLogNotice() << "Loaded Painter: Christel";
	}
